...
if (
    ...
    $estado = intval($_POST["rela_estado_decoraciones"]);

    try {
        $pdo = getConexion();
        $sql = "UPDATE tematica SET tematica_descripcion = :tematica_descripcion, rela_estado_decoraciones = :estado WHERE id_tematica = :id_tematica";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([
            'tematica_descripcion' => $tematica_descripcion,
            'estado_decoraciones_descri' => $estado,
